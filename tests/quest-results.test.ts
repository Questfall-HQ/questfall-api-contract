...
test('history documents result links on closing events for the current response',()=>{
 for(const name of ['AuthorQuestHistory'])expect(schemas.$defs[name].properties.items.items.properties.publication_id.description).toContain('Closing events');
});
