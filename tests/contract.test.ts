...
describe('contract manifest', () => {
  ...
  test('is internally valid and indexable', () => {
    ...
    expect(Object.keys(operations)).toHaveLength(contract.routes.length)
		expect(contract.routes.length).toBe(167)
  })
  ...
  test('exports stable public schemas', () => {
    ...
    expect(schema('AuthorSpaceBalanceEntryType').enum).toEqual(['transfer', 'donation', 'quest_publication'])
		...
		expect(schema('QuestViewerState').properties.outcome.oneOf[0].$ref).toBe('#/$defs/QuestOutcome')
		expect(schema('QuestOutcome').properties.attempts.items.$ref).toBe('#/$defs/QuestAttempt')
		expect(schema('QuestOutcomeStats').properties.votes.items.$ref).toBe('#/$defs/QuestVoteResult')
		...
		expect(schema('ModerationAssignment').properties.quest_cover.type).toBe('string')
		expect(schema('ModerationAssignment').properties.participant_profile.anyOf[0].$ref).toBe('#/$defs/ModerationParticipant')
		expect(schema('ModerationParticipant').required).toEqual(['id', 'name', 'avatar', 'avatar_icon', 'level'])
